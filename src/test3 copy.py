...
def compute_ground(freqs, impedance, fmin, fmax):
    ...
    print("len window right", len(window_right))
    k = 0.0001
    for i in range(fundamental_freq, fmax, fundamental_freq):

        il = get_closest_index(freqs, i-fundamental_freq+1)
        if i+fundamental_freq>=freqs[-1]:
            ir = len(freqs)-1
        else:
            ir=i+fundamental_freq-1
            while freqs[ir]<i+fundamental_freq-1:
                ir+=1

        window_left = impedance[il:ir]
        print("len window left", len(window_left))
...
get_config()["sim.max_error"] = 2
get_config()["sim.fmax"] = fmax
get_config()["sim.grid"] = "even"
