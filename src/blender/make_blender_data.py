...
from blender_didge_shape import shape_from_geo, connect_ends, log, vertex_circle
#inner_shape=[[0,30], [50, 40], [70, 42]]
#outer_shape=[[0,40], [50,50], [70, 52]]
#inner_shape=[[0,30], [50, 40]]
#outer_shape=[[0,40], [50,50]]
...
if __name__=="__main__":
    ...
    infile="/home/jan/workspaces/didge-sound/didge-lab/src/blender/out.json"
    f=open(infile, "r")
    data=json.load(f)
    f.close()
    ...
    log("outer shape")
    data=shape_from_geo(outer_shape, data, n_circle_segments)
    last_vertex_outer=len(data["verts"])
    last_outer_vertex=len(data["verts"])

    start_vertex_inner=len(data["verts"])
    start_edge_inner=len(data["edges"])
    log("inner shape")
    data=shape_from_geo(inner_shape, data, n_circle_segments)
    last_inner_vertex=len(data["verts"])

    data=muster(data, start_vertex_outer, last_vertex_outer, n_circle_segments)

    data=connect_ends(data, start_vertex_inner, start_vertex_outer, start_edge_inner, start_edge_outer, n_circle_segments, last_inner_vertex, last_outer_vertex)

    f=open("didge_data.json", "w")
